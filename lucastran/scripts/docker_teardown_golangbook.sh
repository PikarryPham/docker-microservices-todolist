...
echo ""
echo -e "${CYANBOLD}Stopping all golangbook containers:${RESET}"
echo -e "  Docker stop command: ${GREENBOLD}docker stop \$(docker ps | grep -e golangbook- -e docker-react- | awk '{print \$1}')${RESET}"
echo ""
echo ""

ERROR=$(docker stop $(docker ps | grep -e golangbook- -e docker-react-| awk '{print $1}') 2>&1 1>/dev/null)
status=$?
if test $status -ne 0
then
