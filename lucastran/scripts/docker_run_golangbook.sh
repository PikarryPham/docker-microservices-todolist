...
if [ "$DOCKERIMAGE" = "" ]; then
  echo ""
  echo -e "${REDBOLD}Please set your DOCKERIMAGE environment variable in the .env file: ${RESET} E.g.: ${CYANBOLD}DOCKERIMAGE=lucastran/golangbook${RESET}"
  echo ""
  exit 0
fi
...
echo -e "  Docker run command: ${GREENBOLD}docker run -d --rm -p ${CONSUL_HTTP_PORT}:${CONSUL_HTTP_PORT} -p 8600:8600/udp --name=golangbook-consul consul:1.15 agent -server -ui -node=consul -bootstrap-expect=1 -client=0.0.0.0${RESET}"
docker run -d --rm -p ${CONSUL_HTTP_PORT}:${CONSUL_HTTP_PORT} -p 8600:8600/udp --name=golangbook-consul consul:1.15 agent -server -ui -node=consul -bootstrap-expect=1 -client=0.0.0.0 2>&1 1>/dev/null
echo -e "  ${GREENBOLD}Complete!${RESET}"
echo ""

echo -e "${CYANBOLD}Starting Fabio container:${RESET}"
echo -e "  Docker run command: ${GREENBOLD}docker run -d --rm -p ${FABIO_HTTP_PORT}:${FABIO_HTTP_PORT} -p ${FABIO_DASHBOARD_PORT}:${FABIO_DASHBOARD_PORT} -v ./fabio.properties:/etc/fabio/fabio.properties --name=golangbook-fabiolb fabiolb/fabio${RESET}"
docker run -d --rm -p ${FABIO_HTTP_PORT}:${FABIO_HTTP_PORT} -p ${FABIO_DASHBOARD_PORT}:${FABIO_DASHBOARD_PORT} -v ./fabio.properties:/etc/fabio/fabio.properties --name=golangbook-fabiolb fabiolb/fabio 2>&1 1>/dev/null
echo -e "  ${GREENBOLD}Complete!${RESET}"
echo ""
...
do 
  ...
  echo -e "  Docker run command: ${GREENBOLD}docker run -d --env-file .env --rm  -p $DYNAMIC_DOCKER_PORT:$SERVERPORT --name golangbook-hello-${INSTANCE} -e DOCKERPORT=${DYNAMIC_DOCKER_PORT} -e CONSUL_HTTP_ADDR=${IPADDRESS}:${CONSUL_HTTP_PORT} -e FABIO_HTTP_ADDR=${IPADDRESS}:${FABIO_HTTP_PORT} $DOCKERIMAGE${RESET}"
  docker run -d --env-file .env --rm -p $DYNAMIC_DOCKER_PORT:$SERVERPORT --name golangbook-hello-${INSTANCE} -e DOCKERPORT=${DYNAMIC_DOCKER_PORT} -e CONSUL_HTTP_ADDR=${IPADDRESS}:${CONSUL_HTTP_PORT} -e FABIO_HTTP_ADDR=${IPADDRESS}:${FABIO_HTTP_PORT} $DOCKERIMAGE 2>&1 1>/dev/null
  echo -e "  ${GREENBOLD}Complete!${RESET}"
  echo ""
...
echo -e "  ${CYANBOLD}Fabio Load Balanced Endpoint is:${RESET} http://${IPADDRESS}:${FABIO_HTTP_PORT} ${RESET}"
echo ""
echo ""

docker build -t docker-react-image:1.0 ./client
