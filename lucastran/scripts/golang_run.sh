...
echo -e "${CYANBOLD}Building Go app:${RESET} ${GREENBOLD}go build -o bin/golangbook .${RESET}"
go build -o bin/golangbook .
status=$?
if test $status -ne 0
then
...
echo -e "${CYANBOLD}Running Go app:${RESET} ${GREENBOLD}./bin/golangbook${RESET}"
./bin/golangbook